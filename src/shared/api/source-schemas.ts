import { z } from 'zod';
import { DemoSourceId, SourceId } from '../kernel/value-objects.js';
...
export const SourceListItem = z.object({ id:z.uuid().transform(SourceId),name:z.string(),kind:z.string(),origin:z.enum(['customer','demo']),status:z.string(),error:z.string().nullable(),landingStrategy:landingStrategySchema.nullable(),filingCount:z.number().int().nullable(),elementCount:z.number().int(),undecidedCount:z.number().int(),lastIntrospectedAt:z.iso.datetime({offset:true}).nullable() });
