...
export function useIntrospections(project:string,source:string){return useInfiniteQuery({queryKey:introspectionKeys.list(project,source),initialPageParam:null as string|null,queryFn:({pageParam})=>request({path:`/api/v1/projects/${project}/sources/${source}/introspections${pageParam?`?cursor=${encodeURIComponent(pageParam)}`:''}`,response:IntrospectionRunList}),getNextPageParam:page=>page.nextCursor,refetchInterval:q=>q.state.data?.pages.some(p=>p.items.some(r=>active(r.state)))?5000:false,staleTime:5000,retry:false});}
export function useIntrospection(project:string,id:string){const cache=useQueryClient();const completed=useRef<string|null>(null);const query=useQuery<RunView,AppError>({queryKey:introspectionKeys.detail(project,id),queryFn:()=>request({path:`/api/v1/projects/${project}/introspections/${id}`,response:IntrospectionRunView}),refetchInterval:q=>q.state.data&&active(q.state.data.state)?5000:false,staleTime:5000,retry:false});
 useEffect(()=>{if(query.data?.state==='complete'&&completed.current!==id){completed.current=id;void cache.invalidateQueries({queryKey:elementKeys.all(project)});void cache.invalidateQueries({queryKey:sourceKeys.detail(project,query.data.sourceId)});}},[cache,id,project,query.data]);return query;
}
