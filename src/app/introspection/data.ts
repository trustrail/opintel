...
export function useIntrospection(project:string,id:string,enabled=true){return useQuery<RunView,AppError>({queryKey:introspectionKeys.detail(project,id),enabled,queryFn:()=>request({path:`/api/v1/projects/${project}/introspections/${id}`,response:IntrospectionRunView}),staleTime:5000,retry:false});}
