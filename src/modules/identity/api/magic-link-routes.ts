import { z } from 'zod';
import { defineRoute, errorEnvelopeSchema, type PublicRequest } from '../../../platform/http/index.js';
...
type PublicMagicLinkRequest<TBody> = PublicRequest<TBody> & { readonly params: Record<string, never> };
...
export function magicLinkRoutes(service: MagicLinkService) {
  return [
    defineRoute({
      ...
      handle: async (request: PublicMagicLinkRequest<z.infer<typeof requestLinkBody>>) => {
    ...
    defineRoute({
      ...
      handle: async (request: PublicMagicLinkRequest<z.infer<typeof callbackBody>>) => {
    ...
    defineRoute({
      ...
      handle: async (request: PublicMagicLinkRequest<z.infer<typeof confirmBody>>) => {
        const meta = requestMetadata(request.headers);
