...
import { landingReceiptSchema } from '../../../shared/landing-contract.js';
import type { AcceptLandingReceipt } from '../application/landing-receipts.js';

export function createLandingReceiptServer(tls: { ca: string; cert: string; key: string; pinnedCertificate: string }, service: AcceptLandingReceipt) {
  const route = { path: '/landing-receipt', permission: 'pinned_sidecar_certificate' } as const;
  if (route.permission !== 'pinned_sidecar_certificate') throw new Error('Receipt route must declare its certificate permission.');
  ...
  return createServer({ ...tls, minVersion: 'TLSv1.3', requestCert: true, rejectUnauthorized: true }, (req, res) => {
    ...
    if (req.method !== 'POST' || req.url !== route.path) { req.resume(); fail(404, 'not_found', 'The endpoint does not exist.'); return; }
    ...
    void (async () => {
      try {
        ...
        if (!parsed.success) { fail(400, 'validation_failed', 'The receipt did not pass validation.'); return; }
