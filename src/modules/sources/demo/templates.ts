import { DeploymentRef } from '../../../shared/api/source-schemas.js';
...
const templateSchema = z.object({ id: z.uuid().transform(DemoSourceId), name: z.string(), narrative: z.string().nullable(), schemaSpec: schemaSpecSchema, generatorSpec: generatorSpecSchema, packVersion: z.number().int(), deploymentRef: DeploymentRef });
export async function readDemoTemplate(industryId: IndustryId, templateId: DemoSourceId) {
  return withPlatform(async (tx) => {
    const [row] = await tx.query(`SELECT id,name,narrative,schema_spec AS "schemaSpec",generator_spec AS "generatorSpec",pack_version AS "packVersion",deployment_ref AS "deploymentRef"
