...
export class PostgresIntrospectionQuery implements IntrospectionQuery {
 ...
 private async view(tx:Tx,row:Row):Promise<RunView>{
  const legacyIds=[...new Set(row.diff.filter(entry=>!('duckdbName'in entry)).map(entry=>entry.elementId??entry.objectId))];
  const names=legacyIds.length?await tx.query<{id:string;name:string|null}>(`SELECT e.id,e.duckdb_name AS name FROM catalog_element e JOIN catalog_object o ON o.id=e.object_id WHERE o.source_id=$1 AND e.id=ANY($2::uuid[]) UNION ALL SELECT id,duckdb_name FROM catalog_object WHERE source_id=$1 AND id=ANY($2::uuid[])`,[row.sourceId,legacyIds]):[];
  const diff=row.state==='complete'?row.diff.map(entry=>{
   ...
   return {change,elementId:entry.elementId??null,duckdbName:'duckdbName'in entry?entry.duckdbName??null:names.find(n=>n.id===(entry.elementId??entry.objectId))?.name??null,
