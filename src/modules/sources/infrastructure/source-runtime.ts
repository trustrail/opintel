...
export function createSourceRuntime(options:SidecarOptions){
 ...
 return new SourceRegistrationService(new PostgresSourceRegistrationRepository(),ids,connector,jobs,async(ctx,id,message)=>{const run=await store.read(ctx,id);if(run.ok&&run.value.state==='queued')await store.advance(ctx,id,'queued','connecting');await store.fail(ctx,id,message,false);});
}
