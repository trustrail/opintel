...
import { validateTokenizedTemporal, validateTokenDeclarations, type TemporalDeclarations, type ExposedType } from '../../catalog/index.js';
...
export class PostgresEntitlements implements EntitlementRepository {
 ...
 set(ctx: EntitlementContext, decision: Entitlement) {
  ...
  return withTenant(ctx,async tx=>{
   ...
   const rows=await tx.query<{id:ElementId;tokenDomain:string|null;caseInsensitive:boolean|null;canon_id:string|null;exposed_type:ExposedType|null;sourceTimezone:string|null;epochUnit:TemporalDeclarations['epochUnit']}>(`SELECT e.id,e.token_domain AS "tokenDomain",e.case_insensitive AS "caseInsensitive",e.canon_id,e.exposed_type,COALESCE(e.source_timezone,d.source_timezone) AS "sourceTimezone",e.epoch_unit AS "epochUnit" FROM catalog_element e JOIN catalog_object o ON o.id=e.object_id JOIN data_source s ON s.id=o.source_id LEFT JOIN catalog_schema_temporal d ON d.source_id=o.source_id AND d.schema_name=o.schema_name WHERE e.id=$1 AND e.status='active' AND o.status='active' AND s.status<>'archived' FOR UPDATE OF e`,[s.elementId]);
