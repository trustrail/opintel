...
import { SecretRef, decodeSecretBytes } from '../../../src/platform/secrets/index.js';
...
export class DevelopmentFileKeyStore implements KeyStore {
 ...
 async write(name:string,hex:string,replace=false){
  const bytes=decodeSecretBytes(SecretRef('secret://custody/'+name),hex);bytes.fill(0);
 ...
 async resolve(ref:SecretRef){const name=ref.slice('secret://custody/'.length);if(!ref.startsWith('secret://custody/'))throw new Error('Invalid custody reference.');const f=await open(this.path(name),constants.O_RDONLY|constants.O_NOFOLLOW);try{return await f.readFile('utf8');}finally{await f.close();}}
 async resolveBytes(ref:SecretRef){return decodeSecretBytes(ref,await this.resolve(ref));}
 async store(path:string,secret:string){await this.write(path,secret);return SecretRef('secret://custody/'+path);}
