...
export type Civil = {year:number;month:number;day:number;hour:number;minute:number;second:number};
export function digits(text:string):number {let value=0;for(const c of text)value=value*10+c.charCodeAt(0)-48;return value;}
...
export function formatSeconds(value:number,fraction:string):string|null {
 ...
 if(day<0||day>=beforeYear(10000))return null;
 let low=1,high=10000;while(low+1<high){const mid=Math.floor((low+high)/2);if(beforeYear(mid)<=day)low=mid;else high=mid;}
 let remainder=day-beforeYear(low),month=1;
 for(const length of lengths(low)){if(remainder<length)break;remainder-=length;month++;}
