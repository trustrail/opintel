...
export class TokenKey {
    ...
    static take(bytes: Uint8Array): Result<TokenKey> {
        try {
            return bytes.byteLength === 32 ? ok(new TokenKey(bytes)) : err(new DomainError('dependency_unavailable', 'The token key must contain exactly 32 raw bytes.'));
        }
        finally {
            bytes.fill(0);
        }
