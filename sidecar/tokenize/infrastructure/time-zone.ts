 ...
 toUtc(local:number,zone:string){
  try{
   ...
   const wall=(instant:number)=>{
    ...
    if(parts.get('era')!=='AD')throw new Error('Outside supported calendar.');
   ...
   const offsets=new Set<number>();for(const delta of [-172800,-86400,0,86400,172800])offsets.add(wall(local+delta)-(local+delta));
  ...
  }catch{return err(new DomainError('validation_failed','The declared timezone or timestamp cannot be resolved.'));}
 }
}
