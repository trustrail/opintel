...
import { periodAsAt, type Cedant, type CedantFileRule } from '../../src/modules/ingest/index.js';
...
export class FilingLander {
  ...
  async process(filing: WatchedFiling, cedant: Cedant | undefined, rule: CedantFileRule | undefined): Promise<WatchedFiling> {
    if (filing.status !== 'ready' || !filing.extraction || filing.landing?.registered) return filing;
    let receipt = filing.landing?.receipt;
    if (!receipt) {
      if (!cedant || !rule || !filing.period || !filing.kind || !filing.cedantId) return { ...filing, status: 'quarantined', reason: 'The original landing attribution is unavailable.' };
      ...
      const landed = await this.writer.land({ source: this.source, filingId: filing.id, cedantId: filing.cedantId, partyCode: cedant.code,
        ...
        columns: filing.extraction.columns }, this.extractor.rows(join(this.directory, filing.path), filing.sha256, cedant, rule));
