...
export class HttpsLandingReceipts implements LandingReceiptPort {
  ...
  async send(receipt: LandingReceipt): Promise<Result<void>> {
    const body = JSON.stringify(landingReceiptSchema.parse(receipt));
    try {
      return await new Promise<Result<void>>((resolve, reject) => {
        const req = request(new URL('/landing-receipt', this.url), { method: 'POST', ...this.tls, agent: false, minVersion: 'TLSv1.3', rejectUnauthorized: true,
          signal: AbortSignal.timeout(9000), headers: { 'content-type': 'application/json', 'content-length': Buffer.byteLength(body) },
