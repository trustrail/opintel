...
describe('landing receipt over pinned mutual TLS', () => {
  ...
  beforeEach(async () => {
    ...
    await withTenant(context, (tx) => tx.query("INSERT INTO data_source (id,project_id,kind,name,credential_ref,receives_landings) VALUES ($1,$2,'postgres','Landings','vault://test/customer',true)", [sourceId, context.projectId]));
  });
