...
const checked = (allowed = true) => ({ allowed, token: 'migration-test' as AuthorizationRevision, checkedAt: Timestamp(new Date()), snapshotAgeMs: 0 });
...
integration('industry migration with Postgres', () => {
  ...
  it('E2-026: industry migration preserves every entitlement field, row by row',async()=>{
    ...
    await scopes.withTenant(ctx,async tx=>{
      const [source]=await tx.query<{id:string}>("INSERT INTO data_source(project_id,name,exposed_alias,kind,credential_ref) VALUES($1,'Warehouse','warehouse','postgres','secret://test/source') RETURNING id",[project]);
  ...
  it('E2-027: industry migration preserves every source, pool, key and binding row', async () => {
    ...
    await scopes.withTenant(ctx,async tx=>{
      await tx.query("INSERT INTO data_source(id,project_id,name,exposed_alias,kind,credential_ref) VALUES($1,$2,'Warehouse','warehouse','postgres','secret://test/warehouse')",[source,project]);
