...
beforeEach(async () => {
  ...
  await withTenant(ctx,(tx) => tx.query("INSERT INTO data_source(id,project_id,kind,name,credential_ref,status) VALUES($1,$2,'postgres','Warehouse','vault://test/source','connected')",[sourceId,ctx.projectId]));
  discovery=snapshot();
