  ...
  beforeEach(async () => {
    ...
    await withTenant(ctx, async tx => {
      ...
      for (const s of [source, second]) {
        ...
        for (const [name,type,duck] of elements) await tx.query("INSERT INTO catalog_element(id,project_id,object_id,source_identifier,source_type,exposed_name,exposed_type,token_domain) VALUES($1,$2,$3,$4,$5,lower($4),$6,'value1')", [ids[name],ctx.projectId,object!.id,name,type,duck]);
      }
    });
  });
